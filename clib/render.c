...
struct texture {
	...
	int memsize;
	uint8_t param[MAX_TEXTURE_PARAM];
};
...
compile(struct render *R, const char * source, int type) {
	...
	if (status == GL_FALSE) {
		...
		log_printf(R->log_ud, "compile failed:%s\n"
			"source:\n %s\n",
			buf, source);
...
link(struct render *R, GLuint prog) {
	...
	if (status == 0) {
		...
		log_printf(R->log_ud, "link failed:%s\n", buf);
...
compile_link(struct render *R, struct shader *s, const char * VS, const char *FS) {
	...
	if (fs == 0) {
		log_printf(R->log_ud, "Can't compile fragment shader");
		return 0;
	} else {
	...
	if (vs == 0) {
		log_printf(R->log_ud, "Can't compile vertex shader");
		return 0;
	} else {
	...
	if (R->attrib_layout == 0) {
		log_printf(R->log_ud, "Set attrib layout first");
		return 0;
	}
	...
	for (i=0;i<a->n;i++) {
		...
		case 4:
			...
			break;
		default:
			log_printf(R->log_ud, "Invalid size %d at attrib %d", va->size, i+1);
			return 0;
		}
	}
...
render_shader_create(struct render *R, struct shader_init_args *args) {
	...
	for (i=0;i<s->texture_n;i++) {
		...
		if (s->texture_uniform[i] < 0) {
			log_printf(R->log_ud, "Need texture uniform %d (%s)", i, args->texture_uniform[i]);
		}
	}
...
render_texture_create(struct render *R, int width, int height, enum TEXTURE_FORMAT format, enum TEXTURE_TYPE type, int mipmap) {
	...
	tex->mipmap = mipmap;
	memset(tex->param, 0, sizeof(tex->param));
...
void 
render_texture_setparam(struct render *R, RID id, enum TEXTURE_PARAM what, enum TEXTURE_PARAM value) {
	assert(what >=0 && what < MAX_TEXTURE_PARAM);
	struct texture * tex = (struct texture *)array_ref(&R->texture, id);
	if (tex == NULL)
		return;
	tex->param[what] = value;
}
...
static int
wrap_param(enum TEXTURE_PARAM p) {
	switch(p) {
	case TEXTURE_REPEAT:
		return GL_REPEAT;
	case TEXTURE_MIRRORED_REPEAT:
		return GL_MIRRORED_REPEAT;
	case TEXTURE_CLAMP_TO_EDGE:
	default:
...
render_texture_update(struct render *R, RID id, int width, int height, const void *pixels, int slice, int miplevel) {
	...
	if (tex->param[TEXTURE_MIN_FILTER] == TEXTURE_LINEAR) {
		if (tex->mipmap) {
	...
	glTexParameteri( type, GL_TEXTURE_MAG_FILTER,
		tex->param[TEXTURE_MAG_FILTER] ? GL_NEAREST : GL_LINEAR );
	
	glTexParameteri( type, GL_TEXTURE_WRAP_S, wrap_param(tex->param[TEXTURE_WRAP_S]));
	glTexParameteri( type, GL_TEXTURE_WRAP_T, wrap_param(tex->param[TEXTURE_WRAP_T]));
